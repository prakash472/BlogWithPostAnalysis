...
class _Review_Classifier_Service():
    ...
    def padding_tokenize_reviews(self, tokenized_reviews):
        tokens_with_paading = tf.keras.preprocessing.sequence.pad_sequences(
            tokenized_reviews, maxlen=MAX_LENGTH, padding="post", truncating="post", value=0)
    ...
    def customizing_input(self, input_text):
        ...
        tokenize_input_text_with_padding = self.padding_tokenize_reviews(
            tokenize_input_text)
...
def Review_Classifier_Service():
    if _Review_Classifier_Service._isinstance is None:
        ...
        _Review_Classifier_Service._sentiment_bert_model = keras.models.load_model(
            SAVED_MODEL_PATH, custom_objects={"BertModelLayer": bert.model.BertModelLayer})
